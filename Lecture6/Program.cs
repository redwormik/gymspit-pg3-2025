...
static void Push(int[] array, int value)
{
	for (int i = 0; i < array.Length; i++) {
		if (array[i] == 0) { // předpokládáme, že 0 je "prázdná" hodnota
			array[i] = value;
...
// Referenční typy
int[] array = new int[5]; // pole je referenční typ
PrintArray(array);
Push(array, 10);
Push(array, 20);
Push(array, 30);
